...
# =============================================================================
# Multi-Architecture Docker Build Script for products-web
# 
...
OPTIONS:
    ...
    -n, --name NAME         Image name (default: products-web)
...
EXAMPLES:
    ...
    $0 --native                     # Build for native platform (avoids emulation)
...
setup_buildx() {
    if [[ "$PLATFORMS" == *","* ]]; then
        ...
        if ! docker buildx ls | grep -q "products-web-builder"; then
            print_info "Creating new buildx builder: products-web-builder"
            docker buildx create --name products-web-builder --driver docker-container --bootstrap
        fi
        
        # Use the builder
        docker buildx use products-web-builder
...
# Main function
main() {
    print_info "Starting multi-architecture Docker build for products-web"
